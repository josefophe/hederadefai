...
import { addSignal, readSignals, findSignalById } from "./utils/utils";
import { x402Fetch } from "./utils/x402Client";
...
app.post("/signal", async (req, res) => {
  try {
    ...
    const paymentResult = await x402Fetch(payment, { to: SERVER_ACCOUNT });
...
app.post("/read-signal", async (req, res) => {
  try {
    ...
    const paymentResult = await x402Fetch(payment, { to: signal.accountId });
    if (!paymentResult.success)
      return res.status(402).json({ error: "Payment failed or insufficient" });
