...
export async function sendUSDTCommand(bot: TelegramBot, msg: TelegramBot.Message) {
  ...
  try {
    ...
    const decimals = 6; // USDT standard
    ...
    try {
      ...
      const signedAssocTx = recipientPrivateKey
        ...
        : assocTx.sign(senderKey); // sender pays fee if recipient key not available
    ...
    // ============================
    await sendToken(client, tokenId, senderAccount, recipientAccountId, amount, senderKey);

    const explorerUrl = `https://hashscan.io/${HEDERA_NETWORK}/transaction/${new Date().getTime()}`; // Optional: update with real txId in sendToken
